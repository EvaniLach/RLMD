...
class AtomSetSelector(TreeNodeSetSelector):
    ...
    def processListItem(self, nodes, item, sets=None):
        ...
        elif item in self.atomList.keys() or item in self.NAatomList.keys():
    ...
    def getNamedAtomSet(self, nodes, item):
        ...
        aalist = {}
        nuklist = {}
        for chain, atoms in perChain.items():
            if chain.isDna():
        ...
        if len(aalist):
            ...
            atoms.extend([x for x in nodes if aalist.has_key(x.name) and \
                          AAnames.has_key(x.parent.type.upper())])
        if len(nuklist):
            atoms.extend([x for x in nodes if nuklist.has_key(x.name)])
        hs = []
