...
class Environment(gym.Env):
    ...
    def __init__(self, render_mode=None, size=5):
        ...
        self.action_space = spaces.Box(low=-10, high=10, shape=(1, 3), dtype=np.float32)
    ...
    def step(self, action):
        ...
        print(self.state[self.state[:,3] == 1])
        # Apply action (only translation for now)
        self.state[self.state[:,3] == 1] += action
