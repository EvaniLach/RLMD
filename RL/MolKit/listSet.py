...
#
# $Header: /opt/cvs/python/packages/share1.5/MolKit/listSet.py,v 1.32.4.1 2015/08/29 01:26:36 sanner Exp $
#
# $Id: listSet.py,v 1.32.4.1 2015/08/29 01:26:36 sanner Exp $
#
...
#                        self.data[i].__dict__[member] = value[i]
    ...
    def remove(self, item):
        if self.elementType is not None:
            assert isinstance(item, self.elementType), \
                   "self=%s\nitem=%s\n, elementType=%s\n"%(
                self.__class__, item.__class__, self.elementType)
