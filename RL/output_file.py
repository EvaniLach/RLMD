...
##          print 'FreeParser'
    ...
    def getKeys(self):
        if self.allLines:
            ...
            self.keys = [x[:6].strip() for x in self.allLines]
...
You have to provide one as the second argument' % key);
    ...
    def parse_PDB_CRYST1(self, lines):
        ...
        self.mol.spaceGroup = lines[0][55:66].strip()
        try:
    ...
    def parse_PDB_ATOM_record(self, rec, mol):
        ...
        resName = rec[17:20]
        resSeq = rec[22:26].strip() #WARNING reSeq is a STRING
    ...
    def getPDBAtomName(self, name, element, charge):
        ...
        #print name.strip(), element, charge
...
##                         rec = atoms[0]
##                         nextResName= rec[17:20] + rec[22:26].strip()
...
#        atom.conformation = 0
        ...
        if self.altLoc :
            ...
            if len(mol.curRes.atoms)>1:
                ...
                # You have to go to the one before.
                lastAtom = mol.curRes.atoms[-2]
                atom.altname = lastAtom.name, '@'.split()[0]
                if name, '@'.split()[0] == atom.altname:
                    # Add the new alternate atom to the LastAtom.alternate and
                    # add the lastAtom to the atom.alternate.
                    lastAtom.alternate.append(atom)
                    atom.alternate.append(lastAtom)
...
class PdbqtParser(PdbqParser):
            ...
            for i in range(ctr):
                ...
                if ll[0]=='BEGIN_RES':
                    if len(ll)==4:
                        ...
                        res_name = ll[1]+ll[2]
                        chain_id = ' ' 
                keys = [x[:6].strip() for x in res_lines]
    ...
    def parse_PDB_TORSDOF(self, rec):
        ...
        for rr in rec:
            if not hasattr(self.mol, 'TORSDOF'):
                self.mol.TORSDOF = int(rr.split()[1])
            else:
                self.mol.TORSDOF += int(rr.split()[1])
    ...
    def parse_PDB_ATOM_record(self, rec, mol):
        ...
        #atom.autodock_element = rec[76:78].strip()
        #5/19:CAUTION:this may break!
        #atom.element = rec[76:78].strip()
        #atom.autodock_element = rec[77:].strip()
...
#        #atom.AtSolPar=float(rec[78:84].strip())
#        #atom.AtVol=float(rec[86:92].strip())
#        atom.AtVol = float(rec[78:84].strip())
#        atom.AtSolPar = float(rec[86:92].strip())
...
# first test split the fisrt chain in 2
    ...
    c2 = mol.chains[0].mol.chains[0].residues[20:].split()
