...
#
# $Header: /opt/cvs/python/packages/share1.5/AutoDockTools/Conformation.py,v 1.48.2.1 2016/02/11 09:24:07 annao Exp $
#
...
class Conformation:
    ...
    def getTorsionOnlyCoords(self):
        """Return your coordinates with no quaternion.
    ...
    def getCoords(self):
        ...
        if not hasattr(self, 'coords') or self.coords is None or len(self.coords)==0:
            ...
            self.coords = numpy.array(self.mol.allAtoms.coords).tolist()
    ...
    def getRMSD_custom(self, refCoords=None):
        ...
        # I know there's a clever way to use numpy to do this...
        for c in c2_coords:
            c[0] = -1.0 * c[0]
            c[2] = -1.0 * c[2]
...
def getAngle(at1, at2, at3 ):
    pt1 = numpy.array(at1.coords, 'f')
    pt2 = numpy.array(at2.coords, 'f')
    pt3 = numpy.array(at3.coords, 'f')
...
class ConformationHandler:
    ...
    """
    def __init__(self, mol, origin, buildAngDict=0):
        """
        """
        self.mol = mol
        self.origin = origin
        ...
        if buildAngDict:
            self.angDict = build_angle_dict(mol)
