...
class Vector:
    ...
    def __div__(self, other):
        if isVector(other):
            ...
            return Vector(numpy.divide(self.array, 1.0 * other))

    def __rdiv__(self, other):
        raise TypeError("Can't divide by a vector")

    def __cmp__(self, other):
        return cmp(numpy.add.reduce(abs(self.array - other.array)), 0)
    ...
    def normal(self):
        ...
        length = numpy.sqrt(numpy.add.reduce(self.array * self.array))
        if length == 0:
            raise ZeroDivisionError("Can't normalize a zero-length vector")
        return Vector(numpy.divide(self.array, length))
    ...
    def dyadicProduct(self, other):
        ...
        if isVector(other):
            return TensorModule.Tensor(self.array, 1) * TensorModule.Tensor(
                other.array, 1
            )
        elif TensorModule.isTensor(other):
    ...
    def angle(self, other):
        ...
        cosa = numpy.add.reduce(self.array * other.array) / numpy.sqrt(
            numpy.add.reduce(self.array * self.array)
