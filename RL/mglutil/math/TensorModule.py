...
class Tensor:
    ...
    def __init__(self, elements, nocheck=None):
        ...
        if nocheck is None:
            if not numpy.logical_and.reduce(
                numpy.equal(numpy.array(self.array.shape), 3)
            ):
    ...
    def __mul__(self, other):
        if isTensor(other):
            a = self.array[self.rank * (slice(None),) + (None,)]
            b = other.array[other.rank * (slice(None),) + (None,)]
            return Tensor(numpy.innerproduct(a, b), 1)
    ...
    def __cmp__(self, other):
        if self.rank != other.rank:
            ...
            return not numpy.logical_and.reduce(
                numpy.equal(self.array, other.array).ravel()
            )
    ...
    def dot(self, other):
        ...
        if isTensor(other):
            a = self.array
            b = numpy.transpose(other.array, list(range(1, other.rank)) + [0])
            return Tensor(numpy.innerproduct(a, b), 1)
        else:
            return Tensor(self.array * other, 1)
    ...
    def transpose(self):
        ...
        return Tensor(numpy.transpose(self.array))

    def symmetricalPart(self):
        "Returns the symmetrical part of a rank-2 tensor."
        if self.rank == 2:
            return Tensor(
                0.5 * (self.array + numpy.transpose(self.array, numpy.array([1, 0]))), 1
            )
        else:
            raise ValueError("Not yet implemented")
    ...
    def diagonalization(self):
        ...
        if self.rank == 2:
            ...
            ev, vectors = linalg.eig(self.array)
    ...
    def inverse(self):
        ...
        if self.rank == 2:
            ...
            return Tensor(linalg.inv(self.array))
        else:
            raise ValueError("Undefined operation")
