...
# Input manual parameters if needed
# opt_list = [('-r', ''), ('-v', ''), ('-o', ''), ('-A', 'None'), ('-C', ''), ('-p', ''), ('-U', 'nphs'), ('', ''), ('', '')]


def main(file_path=None, opt_list=[]):
    # initialize required parameters
    # -s: receptor
    receptor_filename = file_path
    ...
    # -o outputfilename
    outputfilename = output_filename(file_path)
    # -m mode
    mode = "automatic"
    ...
    for o, a in opt_list:
        if o in ("-r", "--r"):
            receptor_filename = a
            if verbose:
                print("set receptor_filename to ", a)
        if o in ("-v", "--v"):
            verbose = True
            if verbose:
